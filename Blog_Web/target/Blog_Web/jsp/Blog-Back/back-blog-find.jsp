...
<head>
    <meta charset="UTF-8">
    <title>JAVA-SSM 个人博客后台管理 —— 查找博文</title>
...
<div class="back-right">
    ...
    <div class="back-right-left-nav">
        <ul>
            ...
            <li><a href="#">修改博文</a></li>
            <li><a href="#"  id="color-black">查找博文</a></li>
        </ul>
    </div>
    <div class="back-comment">
        ...
        <table>
            ...
            <c:forEach items="${findList}" var="blog">
                <tr>
                    <td>${blog.id}</td>
                    <td>
                        <c:choose>
                    ...
                    <td>
                        ...
                        <a href="<c:url value="/blog/deleteBlogByTitle.action?id=${blog.id}&title=${title}"/>" onclick="return delConfirm()" class="blog-delete">删除</a>
                    </td>
                </tr>
            </c:forEach>
            <tr>
                <td colspan="8" style="border: none;padding: 20px 0;text-align: right">
                    <ul class="pagination">
                        <li>
                            <a href="<c:url value="/blog/findBlogByName.action?pageNum=${page.firstPage}&title=${title}"/> ">F</a>
                        </li>
                        <li>
                            <c:choose>
                                <c:when test="${page.hasPreviousPage}">
                                    <a href="<c:url value="/blog/findBlogByName.action?pageNum=${page.prePage}&title=${title}"/> ">&laquo;</a>
                                </c:when>
                                <c:otherwise>
                        ...
                        <c:forEach var="i" begin="1" end="${page.pages}">
                            <c:if test="${page.pageNum ==i}">
                                <li  style="background-color: darkseagreen;">
                                    <a href="<c:url value="/blog/findBlogByName.action?pageNum=${i}&title=${title}"/>"
                                    >${i}</a>
                                </li>
                            </c:if>
                            <c:if test="${page.pageNum !=i}">
                                <li>
                                    <a href="<c:url value="/blog/findBlogByName.action?pageNum=${i}&title=${title}"/>">${i}</a>
                                </li>
                            </c:if>
                        </c:forEach>
                        <li>
                            <c:choose>
                                <c:when test="${page.hasNextPage}">
                                    <a href="<c:url value="/blog/findBlogByName.action?pageNum=${page.nextPage}&title=${title}"/> ">&raquo;</a>
                                </c:when>
                                <c:otherwise>
                        ...
                        <li>
                            <a href="<c:url value="/blog/findBlogByName.action?pageNum=${page.lastPage}&title=${title}"/> ">L</a>
                        </li>
                    </ul>
                </td>
            </tr>
