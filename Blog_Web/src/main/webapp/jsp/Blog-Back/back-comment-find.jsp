...
<head>
    <meta charset="UTF-8">
    <title>JAVA-SSM 个人博客后台管理 —— 查找评论</title>
...
<div class="back-right">
    ...
    <div class="back-right-left-nav">
        <ul>
            <li><a href="<c:url value="/comment/commentinfo.action"/>" >评论管理</a></li>

            <li><a href="#" id="color-black">评论查询</a></li>
        </ul>
    </div>
    <div class="back-comment">
        ...
        <table>
            ...
            <c:forEach items="${findList}" var="comment">
                <tr>
                    <td>${comment.id}</td>
                    <td>
                        <c:choose>
                    ...
                    <td>
                        <a href="<c:url value="/comment/deleteCommentByFind.action?id=${comment.id}&content=${content}"/> " onclick="return delConfirm()" class="delete">删除</a>
                    </td>
                </tr>
            </c:forEach>
            <tr>
                <td colspan="8" style="border: none;padding: 20px 0;text-align: right">
                    <ul class="pagination">
                        <li>
                            <a href="<c:url value="/comment/selectCommentByContent.action?pageNum=${page.firstPage}&content=${content}"/> ">F</a>
                        </li>
                        <li>
                            <c:choose>
                                <c:when test="${page.hasPreviousPage}">
                                    <a href="<c:url value="/comment/selectCommentByContent.action?pageNum=${page.prePage}&content=${content}"/> ">&laquo;</a>
                                </c:when>
                                <c:otherwise>
                        ...
                        <c:forEach var="i" begin="1" end="${page.pages}">
                            <c:if test="${page.pageNum ==i}">
                                <li  style="background-color: darkseagreen;">
                                    <a href="<c:url value="/comment/selectCommentByContent.action?pageNum=${i}&content=${content}"/>"
                                    >${i}</a>
                                </li>
                            </c:if>
                            <c:if test="${page.pageNum !=i}">
                                <li>
                                    <a href="<c:url value="/comment/selectCommentByContent.action?pageNum=${i}&content=${content}"/>">${i}</a>
                                </li>
                            </c:if>
                        </c:forEach>
                        <li>
                            <c:choose>
                                <c:when test="${page.hasNextPage}">
                                    <a href="<c:url value="/comment/selectCommentByContent.action?pageNum=${page.nextPage}&content=${content}"/> ">&raquo;</a>
                                </c:when>
                                <c:otherwise>
                        ...
                        <li>
                            <a href="<c:url value="/comment/selectCommentByContent.action?pageNum=${page.lastPage}&content=${content}"/> ">L</a>
                        </li>
                    </ul>
                </td>
            </tr>
