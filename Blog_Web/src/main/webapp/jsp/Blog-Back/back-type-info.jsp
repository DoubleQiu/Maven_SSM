...
<head>
    <meta charset="UTF-8">
    <title>JAVA-SSM 个人博客后台管理 —— 分类管理</title>
...
<div class="back-right">
    ...
    <div class="back-right-left-nav">
        <ul>
            <li><a href="<c:url value="/type/typeinfo.action"/>" id="color-black">分类管理</a></li>
    ...
    <div class="back-comment">
        <div class="back-title">
            分类管理
            <form method="post" action="<c:url value="/type/findTypeByName.action" />">
            <div class="search">
                <small>${msg}</small>
                <input type="text" name="name" placeholder="请输入分类名称..." size="25">
        ...
        <table>
            ...
            <tr>
                ...
                <td>
                    ...
                    <a href="<c:url value="/type/deleteType.action?id=${type.id}"/> " class="delete" onclick="return delConfirm();">删除</a>
                </td>
            </tr>
            </c:forEach>
            <tr>
                <td colspan="8" style="border: none;padding: 20px 0;text-align: right">
                    <ul class="pagination">
                        <li>
                            <a href="<c:url value="/type/typeinfo.action?pageNum=${page.firstPage}"/> ">F</a>
                        </li>
                        <li>
                            <c:choose>
                                <c:when test="${page.hasPreviousPage}">
                                    <a href="<c:url value="/type/typeinfo.action?pageNum=${page.prePage}"/> ">&laquo;</a>
                                </c:when>
                                <c:otherwise>
                        ...
                        <c:forEach var="i" begin="1" end="${page.pages}">
                            <c:if test="${page.pageNum ==i}">
                                <li  style="background-color: darkseagreen;">
                                    <a href="<c:url value="/type/typeinfo.action?pageNum=${i}"/>"
                                    >${i}</a>
                                </li>
                            </c:if>
                            <c:if test="${page.pageNum !=i}">
                                <li>
                                    <a href="<c:url value="/type/typeinfo.action?pageNum=${i}"/>">${i}</a>
                                </li>
                            </c:if>
                        </c:forEach>
                        <li>
                            <c:choose>
                                <c:when test="${page.hasNextPage}">
                                    <a href="<c:url value="/type/typeinfo.action?pageNum=${page.nextPage}"/> ">&raquo;</a>
                                </c:when>
                                <c:otherwise>
                        ...
                        <li>
                            <a href="<c:url value="/type/typeinfo.action?pageNum=${page.lastPage}"/> ">L</a>
                        </li>
                    </ul>
                </td>
            </tr>
